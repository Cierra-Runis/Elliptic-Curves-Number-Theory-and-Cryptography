...
现在我们可以明白在射影几何中平行线是如何在无穷远处相交的。设 $ y = m x + b_1, quad y = m x + b_2 $ 是两条非竖直线的方程，且 $b_1 != b_2$。将它们转换为齐次形式，得到 $ y = m x + b_1 z, quad y = m x + b_2 z $

（前面的讨论只考虑了形如 $f(x, y) = 0$ 和 $F(x, y, z) = 0$ 的方程；然而，把这些方程整理成“$n$ 次齐次式 $=$ $n$ 次齐次式”的形式也是完全没有问题的）当我们解这些联立方程以找出它们的交点时，我们得到 $ z = 0, quad y = m x $
