...
#note(variant: "符号说明")[
  符号 $ZZ, FF_q, QQ, RR, CC$ 分别表示整数集、有 $q$ 个元素的有限域、有理数域、实数域和复数域．我们使用 $ZZ_n$（而不是 $ZZ \/ n ZZ$）来表示模 $n$ 的整数集．然而，当 $p$ 是质数，且我们将 $ZZ_p$ 视为域而不是作为群或环来使用时，我们使用 $FF_p$ 这个记号，以与 $FF_q$ 的记法保持一致．注意，$ZZ_p$ 并不表示 $p$ 进整数．我们之所以这样选用，主要出于排版的考虑，因为模 $p$ 的整数频繁出现，而 $p$ 进整数的符号仅在 @chap:hyperelliptic-curves 的少数几个例子中出现（用 $cal(O)_p$ 表示）．$p$ 进有理数表示为 $QQ_p$．
...
#note(variant: "致谢")[
  作者感谢 CRC Press 的 Bob Stern 提议撰写本书并给予鼓励，也感谢 CRC Press 编辑团队在本书准备过程中提供的帮助．Ed Eikenberg、Jim Owings、Susan Schmoyer、Brian Conrad 和 Sam Wagstaff 提出了许多建议，使得手稿得到了极大的改进．当然，仍有提升的空间．欢迎将建议和勘误发送至 #link("mailto:lcw@math.umd.edu")[作者邮箱]．勘误列表将发布在 #link("https://math.umd.edu/~lcw/ellipticcurves.html")[本书的网站] 上．
...
如果金字塔有三层的话，那么这是做不到的，因为这一共有 $1 + 4 + 9 = 14$ 颗炮弹，而这不是一个完全平方数．当然，如果只有一颗球，它能构筑起一个一层高的金字塔，同时也是一个 $1 times 1$ 的正方形．如果没有球，那我们就有一个零层高的金字塔和一个 $0 times 0$ 的正方形．除了这些显然的情况外，还有其他的解吗？我们提议使用一个可以追溯到丢番图时期（约公元前 250 年）的数学方法来找到另一个解．
...
令我们所找的三角形三条边为 $a, b, c$，如 @fig:triangle-area-5-with-abc 所示．因为面积 $a b \/ 2 = 5$，我们所找的有理数 $a, b, c$ 就有 $ a^2 + b^2 = c^2，quad quad a b = 10． $ 简单的变形得到 $ (frac(a + b, 2))^2 = frac(a^2 + 2a b + b^2, 4) = frac(c^2 + 20, 4) = (frac(c, 2))^2 + 5，\ (frac(a - b, 2))^2 = frac(a^2 - 2a b + b^2, 4) = frac(c^2 - 20, 4) = (frac(c, 2))^2 - 5． $ 令 $x = (c \/ 2)^2$，得到 $ x - 5 = (frac(a - b, 2))^2 quad 且 quad x + 5 = (frac(a + b, 2))^2． $ 因此，我们正在寻找一个有理数 $x$，使得 $ x − 5，quad quad x，quad quad x + 5 $ 都是有理数的平方．换句话说，我们希望三个有理数的平方构成一个公差为 $5$ 的等差数列．

假设我们能找到这样的 $x$，那么它们的乘积 $(x - 5)(x)(x + 5) = x^3 - 25x$ 也将必是一个有理数的平方，所以我们需要找方程 $ y^2 = x^3 - 25x $ 的有理解．

正如前文所说，这是椭圆曲线的方程．当然，即使我们找到了一个这样的有理数解，也不能保证它对应一个有理边三角形（见 @exercise:1-2）．不过，一旦我们找到了一个满足 $y != 0$ 的有理数解，就可以利用它构造出另一个确实对应有理边三角形的解（见 @exercise:1-2）．这正是我们接下来要做的事情．

以备未来需要，我们记 $ x = (c/2)^2，quad quad y = sqrt((x - 5)(x)(x + 5)) = frac((a - b)(c)(a + b), 8) = frac((a^2 - b^2) c, 8)． $

该曲线上有三个“显然”的点：$(-5, 0), (0, 0), (5, 0)$，这些点对我们帮助不大．它们不能构成三角形，并且连接其中任意两个点的直线都会与曲线在剩下的那点相交．简单搜索我们可以找到点 $(-4, 6)$，然而这个点与上述三个点任一的连线都无法给出有效的信息．#footnote[译者注：实际计算会分别得到 $(45, 300), (25 \/ 4, -75 \/ 8), (-5 \/ 9, 100 \/ 27)$ 这三个点，均不满足] 唯一剩下的可能就是考虑过点 $(-4, 6)$ 与它自身，也就是曲线在这点的切线．隐函数求导可得 $ 2y y' = 3x^2 - 25，quad quad y' = frac(3x^2 - 25, 2y) = 23/12． $
...
哪些整数 $n$ 能是有理边直角三角形的面积，这个问题被称为 *同余数问题*．如我们上面所看到的另一种表述方式就是：是否存在三个有理数的平方构成公差为 $n$ 的等差数列．这一问题最早出现在大约公元 900 年左右的阿拉伯手稿中．Tunnell 在 20 世纪 80 年代对该问题提出了一个猜想性答案，并加以证明 @ref:tunnell1983diophantine．
...
比如考虑 $n = 5$，此时没有满足 $2x^2 + y^2 + 8z^2 = 5$ 的整数解，故 $z$ 为奇数的解的个数 $= 0 = 0 =$ $z$ 为偶数的解的个数，条件显然成立，于是预测存在面积为 $5$ 的有理边直角三角形．再考虑 $n = 1$，此时 $2x^2 + y^2 + 8z^2 = 1$ 的整数解为 $(0, 1, 0)$ 和 $(0, -1, 0)$，两者都是 $z$ 为偶数，故由 $2 != 0$ 得不存在面积为 $1$ 的有理边直角三角形．这最早由费曼通过它的无穷下降法证明（见 @sec:fermat-infinite-descent）．
...
费马三次方程也可以转换为椭圆曲线．假设 $a^3 + b^3 = c^3$ 且 $a b c != 0$，由于 $a^3 + b^3 = (a + b)(a^2 - a b + b^2)$，我们必须有 $a + b != 0$．令 $ x = 12 frac(c, a + b)，quad quad y = 36 frac(a - b, a + b)． $
...
#exercise[
  + 证明若 $x, y$ 是满足 $y^2 = x^3 - 25x$ 的有理数，且 $x$ 是一个有理数的平方，则这无法推出 $x + 5$ 和 $x - 5$ 都是有理数的平方．（提示：令 $x = 25 \/ 4$）
...
#exercise[
  ...
  + 求出该三次方程的另一个非零根 $t$，并据此计算得到 $x = 1681 \/ 144$ 和 $y = 62279 \/ 1728$．
] <exercise:1-3>
...
#exercise[
  证明变量代换 $x_1 = 12 x + 6, y_1 = 72 y$ 将曲线 $y_1^2 = x_1^3 - 36 x_1$ 转变为 $y = x (x + 1) (2 x + 1) \/ 6$．
...
下面的示例将表明，这种操作并不等同于将两个点的坐标直接相加．也许用 $P_1 #text(fill: colors.blue, $+_E$) P_2$ 这样的记号来表示这种运算会更恰当，但我们选择更简洁的记号，因为我们从不会通过坐标相加的方式来进行点的加运算．

假定 $P_1 != P_2$，且两个都不是 $infinity$，连接 $P_1$ 和 $P_2$ 的直线斜率为 $ m = frac(y_1 - y_2, x_2 - x_1)． $

如果 $x_1 = x_2$，那么 $L$ 是竖直的．我们之后再讨论这种情形，所以先假设 $x_1 != x_2$．那么 $L$ 的方程为 $y = m (x - x_1) + y_1．$

为了找到与 $E$ 的交点，代入得到 $ (m (x - x_1) + y_1)^2 = x^3 + A x + B． $

整理得到如下形式 $ 0 = x^3 - m^2 x^2 + dots.c． $
...
如果已知两个根 $r, s$，就可以得到第三个根 $t = -a - r - s$，在我们的情形中，这意味着 $ x = m^2 - x_1 - x_2 $ 且 $ y = m (x - x_1) + y_1． $

最后关于 $x$ 轴翻转得到点 $P_3 = (x_3, y_3)$ 有 $ x_3 = m^2 - x_1 - x_2，quad quad y_3 = m (x_1 - x_3) - y_1． $

若 $x_1 = x_2$ 且 $y_1 != y_2$，说明通过 $P_1$ 和 $P_2$ 的直线是竖直的，它与曲线 $E$ 的交点是 $infinity$．将 $infinity$ 关于 $x$ 轴翻转仍为 $infinity$（这也是我们将 $infinity$ 放在 $y$ 轴顶端与底端的原因）．因此，在这种情况下 $P_1 + P_2 = infinity$．

现在考虑 $P_1 = P_2 = (x_1, y_1)$ 的情况．当两点非常接近时，它们之间的连线趋近于切线，所以在它们完全重合时，我们取过该点的切线作为 $L$．通过隐函数求导我们可以计算出切线的斜率 $m$：$ 2y frac("d"y, "d"x) = 3x^2 + A quad ==> quad m = frac("d"y, "d"x) = frac(3x_1^2 + A, 2y_1)． $
...
我们可以得到 $ 0 = x^3 - m^2 x^2 + dots.c． $

这次我们只知道一个根 $x_1$，但因为 $L$ 是 $E$ 在点 $P_1$ 处的切线，所以 $x_1$ 是一个重根．因此重复上文的过程，我们可以得到 $ x_3 = m^2 - 2 x_1，quad quad y_3 = m (x_1 - x_3) - y_1 $
...
#theorem[
  ...
  换句话说，在 $E$ 上的点构成了一个以 $infinity$ 为单位元的加法阿贝尔群．
]

#proof[
  交换律的证明是显然的，因为连接 $P_1$ 和 $P_2$ 的直线与连接 $P_2$ 和 $P_1$ 的直线是同一条直线． $infinity$ 的单位元性质由其定义直接得出．对于逆元，令 $P'$ 为 $P$ 关于 $x$ 轴的对称点，那么有 $P + P' = infinity$．

  最后我们需要证明结合律．这是椭圆曲线上点的加法最微妙、且最不直观的性质．事实上，可以为椭圆曲线上的点构造许多满足 1. 2. 3. 性质的运算律，这些律可能比我们当前所定义的加法规则更简单或更复杂．然而，这样的运算律极少能满足结合律．实际上，我们所定义的加法居然满足结合律，确实是一个令人惊讶的事实．毕竟，我们是从两个点 $P_1$ 和 $P_2$ 出发，通过某种操作得到第三个点 $P_1 + P_2$，然后再将其与 $P_3$ 相加得到 $(P_1 + P_2) + P_3$．如果我们反过来先计算 $P_2 + P_3$，再计算 $P_1 + (P_2 + P_3)$，乍一看没有什么明显的理由表明这两个结果给出的是相同的点．
...
椭圆曲线上的点构成阿贝尔群这一事实，是大多数有趣性质与应用背后的基础．于是就产生了一个问题：我们所得到的这些点群，具有什么性质？下面是一些示例：
...
这种方法使我们能够非常快速地计算 $k P$，即使 $k$ 是非常大的整数．唯一的困难在于：如果我们在有理数域中进行计算，点的坐标会增长得非常快（见定理 8.18）．然而，当我们在有限域中计算，例如在 $FF_p$ 上，这就不是问题，因为我们可以持续地模 $p$，从而使参与计算的数值保持相对较小．注意到结合律使我们在计算这些加法时无需担心求和顺序．/// TODO: Ref to 8.18
...
设 $K$ 是一个域．定义在 $K$ 上的二维 *射影空间* $PP^2_K$ 是由三元组 $(x, y, z)$ 的等价类给出，其中 $x, y, z in K$，且至少有一个不为零．如果存在 $lambda in K^times$ 使得 $ (x_1, y_1, z_1) = (lambda x_2, lambda y_2, lambda z_2) $ 则称两个三元组 $(x_1, y_1, z_1)$ 和 $(x_2, y_2, z_2)$ 是 *等价* 的，记作 $(x_1, y_1, z_1) ~ (x_2, y_2, z_2)$ ．等价类只与 $x, y, z$ 的比例有关，故使用 $(x : y : z)$ 表示等价类．#footnote[译者注：推荐视频 #link("https://www.bilibili.com/video/av113974288845476/")[在数学上，分类是什么意思？] 从开头到 4:46 处的内容]
...
#proof[
  将曲线的方程写作 $ u^2 - c^2 = (c^2 d u^2 - 1) v^2 = frac(w^2, c^2 d u^2 - 1) $ 由此得到的曲线为 $ w^2 = c^2 d u^4 - (c^4 d + 1) u^2 + c^2 $ 然后使用 @subsec:quartic-equations 中的公式将其转换为 Weierstrass 形式．加法律的公式可以通过直接计算得到．

  现在我们要证明这个加法律在曲线上的所有点 $C(K)$ 都有定义．换句话说，我们要证明分母不为零．

  假设 $d u_1 v_1 u_2 v_2 = -1$，那么 $u_i, v_i != 0$ 且 $u_1 v_1 = -1 \/ d u_2 v_2$，将其代入曲线 $C$ 的方程，得到 $ u_1^2 + v_1^2 = c^2 (1 + 1/(d u_2^2 v_2^2)) = frac(u_2^2 + v_2^2, d u_2^2 v_2^2) $

  因此 $ (u_1 + v_1)^2 = u_1^2 + v_1^2 + 2 u_1 v_1 = 1/d (frac(u_2^2 + v_2^2 - 2 u_2 v_2, u_2^2 v_2^2)) = 1/d frac((u_2 - v^2)^2, (u_2 v_2)^2) $

  由于 $d$ 不是平方数，这等式必然有 $0 = 0$，因此 $u_1 + v_1 = 0$．

  类似地有 $ (u_1 - v_1)^2 = 1/d frac((u_2 + v_2)^2, (u_2 v_2)^2) $ 得到 $u_1 - v_1 = 0$．因此 $u_1 = v_1 = 0$，导出矛盾．

  同理当 $d u_1 v_1 u_2 v_2 = 1$ 时，点加法对于 $C(K)$ 中的所有点始终有定义．
]

一个有趣的特点是，当 $P_1 != P_2$ 时，$2P$ 和 $P_1 + P_2$ 并没有使用不同的公式．

点加法可以写成投影坐标的形式．使用这种形式进行计算时，无论是两点相加还是点倍加，所需的计算量均为 10 次乘法和 1 次平方．

虽然在代数闭域中，任何椭圆曲线都可以变换为命题中所述的形式，但在基域上往往无法做到这一点．一个简单的判断方法是：该曲线上是否存在阶为 2 的点．实际上，曲线 $C$ 上的点 $(c, 0)$ 的阶为 4（见 @exercise:2-7），所以，只有当曲线在某个域上定义了阶为 4 的点，它才可能在该域上被转化为 Edwards 形式．/// TODO: Check base field.
...
第一个输出表示 $E_1 (QQ)$ 的挠子群的阶为 10，它是一个阶为 10 的循环群，并且由点 $(3, 1944)$ 生成．第二个输出表示 $E_2 (QQ)$ 的挠子群是平凡的．

模素数 $p$ 的椭圆曲线上点个数的形式为 $p + 1 - a_p$．$E_1$ 的 $a_(13)$ 的值可以如下计算：
...
因此，在 $E_1 mod 13$ 上一共有 $13 + 1 - 4 = 10$ 个点．

我们也可以对模 p 的曲线进行计算．令

$ E_3: y^2 = x^3 + 10x + 5 quad (mod 13) $
...
Pari 也能处理与椭圆曲线相关的复函数．例如，第 10.4 节开头提到的 $j((1 + sqrt(-171)) \/ 2)$ 可计算为：
...
```
? ellj((1+sqrt(-171))/2)
%14 = -694282057876536664.012288686708307426044367453641244662629851
      - 7.05609883 E-49*I
```

此时虚部小于 $10^{-48}$．

要查看可用的其他函数，输入 `?`．要查看与椭圆曲线相关的函数，输入 `?5`．要了解某个命令（如 `elladd`）的用法，输入：
...
Magma 是一个大型的计算机代数软件包．使用它需要许可证．它在一些机构的计算机上可用．有关一般信息，请参见 #link("http://magma.maths.usyd.edu.au/magma/")[此链接]．
...
让我们输入示例 9.3 中的椭圆曲线 $E_1 : y^2 = x^3 - 58347 x + 3954150$．向量表示的是广义 Weierstrass 形式中的系数 $[a_1, a_2, a_3, a_4, a_6]$．除非另有说明，曲线是定义在有理数域上的．/// TODO: Ref to example 9.3

```
> E1:= EllipticCurve( [ 0, 0, 0, -58347, 3954150 ]);
```

注意，语句末尾需要加分号．查看 $E_1$ 的定义：

```
> E1;
Elliptic Curve defined by yˆ2 = xˆ3 - 58347*x + 3954150 over Rational Field
```

我们再定义另一个曲线 $E_2 : y^2 = x^3 + 73$，这里我们使用了（非广义的）Weierstrass 形式的简写，即只写出 $[a_4, a_6]$：

```
> E2:= EllipticCurve( [ 0, 73 ]);
```

我们来将点 $(2, 9)$ 和 $(3, 10)$ 相加，这两个点在 $E_2$ 上．写作 `E2![2,9]` 是为了表明 $[2, 9]$ 属于曲线 $E_2$，而不是其他集合．
...
注意结果是投影坐标．我们也可以使用一个或两个投影坐标的点完成计算：
...
计算机记得 S 在 $E_2$ 上，因此我们可以将其和 $E_2$ 上的另一点相加：
